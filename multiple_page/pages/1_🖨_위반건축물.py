...
from rapidfuzz import process  # 변경: rapidfuzz로 대체
...
def get_similar_filenames(input_text, clean_to_raw, max_results=20):
    ...
    # rapidfuzz 기반 유사도 매칭
    matches = process.extract(input_text, clean_filenames, limit=max_results, score_cutoff=0.7)
    # 결과에서 파일명만 추출
...
def main():
    ...
    if selected_clean_filename:
        ...
        if image:
            # 사용자 지정 크롭 크기 설정
            crop_width = 1100  # 크롭할 너비
            crop_height = 900  # 크롭할 높이
